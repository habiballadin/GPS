...
const saveVehicleProfile = async (
  vehicleId: number,
  profile: string,
  config: typeof DEFAULT_FMB920_CONFIG,
  ...
  const response = await fetch(`/api/v1/vehicles/${vehicleId}/profile`, {
    ...
    body: JSON.stringify({
      profile,
      config: { ...config, profile: undefined },
    }),
  });
...
const loadVehicleProfile = async (
  ...
  return response.json() as Promise<{
    profile?: string;
    config?: Partial<typeof DEFAULT_FMB920_CONFIG>;
  } | null>;
};

const deleteVehicleProfile = async (
  vehicleId: number,
  accessToken?: string | null,
) => {
  const token = accessToken ?? localStorage.getItem("gps_access_token");
...
function buildFmb920Commands(config: typeof DEFAULT_FMB920_CONFIG) {
  ...
  if (config.serverIp.trim())
    commands.push(`setparam 2004:${config.serverIp.trim()}`);
  ...
  if (config.heartbeat.trim())
    commands.push(`setparam 3000:${config.heartbeat.trim()}`);
  if (config.distance.trim())
    commands.push(`setparam 3001:${config.distance.trim()}`);
  if (config.sleepMode.trim())
    commands.push(`setparam 3010:${config.sleepMode.trim()}`);
  if (config.timezone.trim())
    commands.push(`setparam 2100:${config.timezone.trim()}`);
  if (config.ignition.trim())
    commands.push(`setparam 4010:${config.ignition.trim()}`);
  if (config.powerSaving.trim())
    commands.push(`setparam 3030:${config.powerSaving.trim()}`);
  if (config.sosEnabled.trim())
    commands.push(`setparam 5001:${config.sosEnabled.trim()}`);
  if (config.sosNumber1.trim())
    commands.push(`setparam 5002:${config.sosNumber1.trim()}`);
  if (config.sosNumber2.trim())
    commands.push(`setparam 5003:${config.sosNumber2.trim()}`);
  if (config.overspeedLimit.trim())
    commands.push(`setparam 3020:${config.overspeedLimit.trim()}`);
  if (config.geofenceRadius.trim())
    commands.push(`setparam 3050:${config.geofenceRadius.trim()}`);
  if (config.movementAlert.trim())
    commands.push(`setparam 3060:${config.movementAlert.trim()}`);
  if (config.movementMinutes.trim())
    commands.push(`setparam 3061:${config.movementMinutes.trim()}`);
  if (config.input1Mode.trim())
    commands.push(`setparam 4101:${config.input1Mode.trim()}`);
  if (config.input2Mode.trim())
    commands.push(`setparam 4102:${config.input2Mode.trim()}`);
  if (config.output1Mode.trim())
    commands.push(`setparam 4201:${config.output1Mode.trim()}`);
  if (config.output2Mode.trim())
    commands.push(`setparam 4202:${config.output2Mode.trim()}`);
...
export function VehicleRegistry() {
  ...
  const [statusFilter, setStatusFilter] = useState<
    "all" | "online" | "offline"
  >("all");
  const [sortBy, setSortBy] = useState<"name" | "last_seen">("name");
  const [detailVehicleId, setDetailVehicleId] = useState<number | null>(null);
  const detailVehicle = vehicles.find((vehicle) => vehicle.id === detailVehicleId);

  const filteredVehicles = useMemo(
    () =>
      vehicles
        .filter((vehicle) => {
          const haystack = `${vehicle.name} ${vehicle.imei}`.toLowerCase();
          const matchesQuery = haystack.includes(query.toLowerCase().trim());
          const matchesStatus =
            statusFilter === "all" ||
            (statusFilter === "online"
              ? Boolean(vehicle.lastSeenAt)
              : !vehicle.lastSeenAt);
        ...
        .sort((a, b) =>
          sortBy === "name"
            ...
            : Number(Boolean(b.lastSeenAt)) - Number(Boolean(a.lastSeenAt)),
        ),
    [vehicles, query, statusFilter, sortBy],
  );
    ...
    if (response.ok) {
      ...
      setVehicles(
        rows.map((row) => ({
          ...
          status: row.active ? "available" : "out_of_service",
  ...
  const submit = async (event: FormEvent) => {
    ...
    setMessage(
      "Device registered. Configure the tracker to connect to TCP 5001 or 5002.",
    );
    await load();
  };
  ...
  const sendSingleCommand = async (id: number, rawCommand: string) => {
    const res = await fetch(`/api/v1/vehicles/${id}/command`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
  ...
  const resetFmb920Config = async (id: number) => {
    ...
    if (deleted) {
      ...
      setConfigResult(
        "Device profile was reset locally, but no saved backend profile was found.",
      );
    }
  };
  ...
  const exportFmb920Profile = () => {
    ...
    setConfigResult(
      `Exported profile JSON for ${cmdVehicle?.name ?? "vehicle"} .`,
    );
  };

  const importFmb920Profile = async (
  ...
  return (
    <section>
      <div className="mb-8 flex items-end justify-between">
        <div>
          ...
          <p className="mt-2 text-slate-500">
            Register the IMEI before powering on a tracker.
          </p>
        </div>
      </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-panel">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-[1fr_150px_150px]">
              ...
              >
                ...
                <option value="last_seen">Sort: Status</option>
              </select>
            </div>
                ...
                <tbody>
                  {filteredVehicles.map((vehicle) => (
                    <tr className="border-b border-slate-100" key={vehicle.id}>
                      ...
                      <td>
                        <span
                          className={`rounded-full px-2 py-1 text-xs font-semibold ${vehicle.lastSeenAt ? "bg-green-50 text-green-700" : "bg-slate-100 text-slate-500"}`}
                        >
                      ...
                      <td className="space-x-3">
                        ...
                        {vehicle.protocol === "teltonika" && (
                          <button
                            ...
                            onClick={() =>
                              void openVehicleCommandPanel(vehicle.id)
                            }
                          >
                            Command
                          </button>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-panel">
              <div className="flex items-center justify-between">
                <h2 className="text-lg font-bold">
                  Send command — {cmdVehicle.name}
                </h2>
                <button
                  className="text-xs text-slate-400 hover:text-slate-600"
                  onClick={() => setCmdVehicleId(null)}
                >
                  ✕ Close
                </button>
              </div>
              <div className="mt-4 flex flex-wrap gap-2">
                {PRESET_COMMANDS.map((p) => (
                  <button
                    key={p.cmd}
                    className="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-semibold hover:bg-mist"
              ...
              <div className="mt-4 flex gap-2">
                <input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter") void sendCommand(cmdVehicle.id);
                  }}
                />
                <button
              ...
              {cmdResult && (
                <pre className="mt-4 rounded-xl bg-slate-50 p-4 text-sm font-mono text-slate-700 whitespace-pre-wrap">
                  {cmdResult}
                </pre>
              )}
              ...
              <div className="mt-6 rounded-2xl border border-forest/20 bg-mist/40 p-4">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="text-xs text-slate-500">
                      Common network and reporting settings
                    </p>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    ...
                    <label className="cursor-pointer rounded-xl border border-slate-300 bg-white px-4 py-2 text-xs font-bold text-slate-600">
                      ...
                      <input
                        ...
                        className="hidden"
                        onChange={(event) => void importFmb920Profile(event)}
                      />
                    </label>
                  </div>
                </div>
                  ...
                  <label className="block text-xs font-semibold text-slate-600">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setFmbConfig((current) => ({
                          ...current,
                          heartbeat: e.target.value.replace(/\D/g, ""),
                        }))
                      }
                      placeholder="60"
                    />
                  ...
                  <label className="block text-xs font-semibold text-slate-600">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setFmbConfig((current) => ({
                          ...current,
                          distance: e.target.value.replace(/\D/g, ""),
                        }))
                      }
                      placeholder="100"
                    />
                  ...
                  <label className="block text-xs font-semibold text-slate-600">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setFmbConfig((current) => ({
                          ...current,
                          timezone: e.target.value.replace(/[^-\d]/g, ""),
                        }))
                      }
                      placeholder="0"
                    />
                  ...
                  <label className="block text-xs font-semibold text-slate-600">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setFmbConfig((current) => ({
                          ...current,
                          overspeedLimit: e.target.value.replace(/\D/g, ""),
                        }))
                      }
                      placeholder="90"
                    />
                  ...
                  <label className="block text-xs font-semibold text-slate-600">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setFmbConfig((current) => ({
                          ...current,
                          geofenceRadius: e.target.value.replace(/\D/g, ""),
                        }))
                      }
                      placeholder="200"
                    />
                  ...
                  <label className="block text-xs font-semibold text-slate-600">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setFmbConfig((current) => ({
                          ...current,
                          movementMinutes: e.target.value.replace(/\D/g, ""),
                        }))
                      }
                      placeholder="10"
                    />
                ...
                {configResult && (
                  <pre className="mt-4 rounded-xl bg-slate-900 p-3 text-xs font-mono text-slate-100 whitespace-pre-wrap">
                    {configResult}
                  </pre>
                )}
              </div>
      ...
      {detailVehicle && (
        <div className="fixed inset-0 z-40" role="dialog" aria-modal="true" aria-label={`${detailVehicle.name} details`}>
          <button aria-label="Close vehicle details" className="absolute inset-0 h-full w-full cursor-default bg-slate-900/30" onClick={() => setDetailVehicleId(null)} />
          <aside className="absolute right-0 top-0 h-full w-full max-w-md overflow-y-auto bg-white p-6 shadow-2xl">
            <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-slate-400">Vehicle details</p><h2 className="mt-2 text-2xl font-bold">{detailVehicle.name}</h2></div><button aria-label="Close" className="rounded-lg border border-slate-200 px-3 py-2 text-sm" onClick={() => setDetailVehicleId(null)}>✕</button></div>
            <div className="mt-6 grid gap-3"><div className="rounded-xl bg-mist p-4"><p className="text-xs text-slate-500">Telemetry status</p><p className="mt-1 font-bold">{detailVehicle.lastSeenAt ? 'Online' : 'Offline'}</p><p className="mt-1 text-xs text-slate-500">{detailVehicle.lastSeenAt ? `Last seen ${new Date(detailVehicle.lastSeenAt).toLocaleString()}` : 'No telemetry received yet'}</p></div><dl className="divide-y divide-slate-100 rounded-xl border border-slate-100"><div className="flex justify-between gap-4 p-3 text-sm"><dt className="text-slate-500">IMEI</dt><dd className="font-mono">{detailVehicle.imei}</dd></div><div className="flex justify-between gap-4 p-3 text-sm"><dt className="text-slate-500">Protocol</dt><dd>{detailVehicle.protocol === 'teltonika' ? 'Teltonika FMB920' : 'CONCOX V5'}</dd></div><div className="flex justify-between gap-4 p-3 text-sm"><dt className="text-slate-500">Status</dt><dd>{detailVehicle.status.replace('_', ' ')}</dd></div></dl></div>
            <div className="mt-6 flex gap-3"><Link href={`/vehicles/${detailVehicle.id}`} className="flex-1 rounded-xl bg-forest px-4 py-3 text-center text-sm font-bold text-white">Open full profile</Link>{detailVehicle.protocol === 'teltonika' && <button onClick={() => { setDetailVehicleId(null); void openVehicleCommandPanel(detailVehicle.id) }} className="rounded-xl border border-slate-200 px-4 py-3 text-sm font-bold">Command</button>}</div>
          </aside>
        </div>
      )}
    </section>
