...
export function AuthProvider({ children }: Readonly<{ children: React.ReactNode }>) {
  ...
  useEffect(() => {
    const access = window.localStorage.getItem('fleet_token'); const refresh = window.localStorage.getItem('fleet_refresh_token');
    setToken(access); setReady(true)
    if (!refresh) return
    const timer = window.setInterval(async () => { const response = await fetch('/api/v1/auth/refresh', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ refresh_token: window.localStorage.getItem('fleet_refresh_token') }) }); if (response.ok) { const next = await response.json() as TokenResponse; window.localStorage.setItem('fleet_token', next.access_token); if (next.refresh_token) window.localStorage.setItem('fleet_refresh_token', next.refresh_token); setToken(next.access_token) } else { window.localStorage.removeItem('fleet_token'); window.localStorage.removeItem('fleet_refresh_token'); setToken(null) } }, 45 * 60 * 1000)
